...
// Fetch ONLY PAID deliveries for the grid
$stmt = $pdo->prepare("SELECT * FROM for_deliveries WHERE status = 'PAID' ORDER BY date_time_sold DESC");
$stmt->execute();
$paidDeliveries = $stmt->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Paid Products | Villaruz Print Shop & General Merchandise</title>
...
<body>
    <div class="app-wrapper">
        ...
        <main class="main-content">
            ...
            <!-- Search Bar (searches only among PAID orders) -->
            ...
            <!-- Orders Grid - Only PAID deliveries -->
            <div class="orders-grid" id="ordersGrid">
                ...
                <?php else: ?>
                    <?php foreach ($paidDeliveries as $delivery):
                        $status = $delivery['status']; // should always be 'PAID'
                        ?>
                        <div class="order-card"
                            ...
                            <div class="status-badge status-paid" id="status-badge-<?php echo $delivery['id']; ?>">
                                <?php echo $status; ?>
                            </div>
                            ...
                            <div class="card-actions">
                                ...
                                <button class="action-btn restore-btn"
                                    onclick="restoreDelivery('<?php echo addslashes($delivery['delivery_number']); ?>', this)"><i
                                        class="fas fa-undo-alt"></i> Restore</button>
                                <a href="../web/selected_data.php?delivery_number=<?php echo urlencode($delivery['delivery_number']); ?>"
                                    class="action-btn" target="_blank"><i class="fas fa-eye"></i>View</a>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
    ...
    <script>
        ...
        // ========== RESTORE DELIVERY ==========
        window.restoreDelivery = async function (deliveryNumber, btn) {
        ...
        document.querySelectorAll('.status-select').forEach(select => {
            select.addEventListener('change', async function () {
                ...
                if (!confirmed) {
                    // revert to original selected value (should be PAID)
                    this.value = 'PAID';
                    return;
                }
